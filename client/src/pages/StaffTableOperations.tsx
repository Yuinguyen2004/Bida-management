...
const estimateBill = (startTime: Date | undefined, pricePerHour: number, now = Date.now()) => {
  if (!startTime) {
    return 0;
  }

  const elapsedMs = Math.max(now - startTime.getTime(), 0);
  return (elapsedMs / 3600000) * pricePerHour;
};
...
export const StaffTableOperations: React.FC<StaffTableOperationsProps> = ({
  ...
  const tableOperations = useMemo<TableData[]>(() => {
    return tables.map((table) => {
      const session = activeSessions.find((activeSession) => {
        const tableId = typeof activeSession.tableId === 'object' ? activeSession.tableId._id : activeSession.tableId;
    ...
    >
            ...
            <div className="table-body">
              {filteredTables.map((table) => {
                ...
                const sessionValue = table.status === 'playing'
                  ? formatCurrency(estimateBill(table.startTime, table.pricePerHour, now))
                  : table.sessionId
                    ? 'Session attached'
                    : 'No active session';
      ...
      {selectedTable && (
        <TableActionModal
          ...
          billAmount={estimateBill(selectedTable.startTime, selectedTable.pricePerHour, now)}
          onTableUpdate={() => {
